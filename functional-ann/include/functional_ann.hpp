...
struct FunctionalANN {
	FunctionalANN(
		...
		} else if (a_fn_primes.size() != num_layers - 1) {
			throw std::runtime_error("a_fn_primes.size() != layer_sizes.size() - 1");
		}
		this->a_fns = a_fns;
	...
	void back_prop_update(const VectorXd& y, double learning_rate) {	
		VectorXd z_prime = loss_fn_prime(a.back(), y).array() * a_fn_primes.back()(a.back()).array();
